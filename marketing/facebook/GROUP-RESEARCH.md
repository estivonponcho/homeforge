...
### 2026-09-14 read-only scan

- **Newly joined:** **F that, I'll just 3D print it!** (private, about 194.7K members). The visible rules require model links to go in comments, require a photo of the printed model, prohibit unapproved sales, and say the main feed is not for printer-support questions. Do not publish a generic HomeForge introduction or external link. Best future fit is an original, photographed functional print with the model link only in comments if requested.
- **Official Home Assistant:** A new question asks whether an Arr stack plus Gluetun or a Portainer-style setup can run on HAOS on a Raspberry Pi 4. Strong contribution fit for Mike's real architecture: keep HAOS appliance-like and run the media/VPN Docker stack on a separate low-power host. A short answer without a HomeForge link is the best trust-building first response.
- **Official Home Assistant:** After Mike's action-time confirmation, Codex posted that no-link architecture answer to the Arr stack + Gluetun question. It recommends keeping HAOS focused on Home Assistant and moving the Docker/VPN stack to a separate Pi, ARM box, or mini PC. Status: LIVE and visibly confirmed. Post permalink: https://www.facebook.com/groups/HomeAssistant/posts/4552827244988610/
- **Official Home Assistant:** A member asked for feedback on the SONOFF NSPanel Pro. Do not answer as hands-on because HomeForge has not documented using it.
- **Official Home Assistant:** A smart-lock automation question already has two correct answers recommending separate service actions for each lock. Skip unless Mike can add tested trace evidence.
- **ESP32/ESP8266 projects:** The newest visible posts were a newly purchased TinkerBlock sensor kit and a kit-for-sale post. Neither has a specific unresolved question that matches a documented HomeForge build, so do not force a comment.
- **HASS / Home Assistant:** The newest visible post was a SONOFF outdoor camera review, not a question. No useful HomeForge contribution identified in that scan.
- **Claude AI Builders:** After Mike's action-time confirmation, Codex answered “Which one AI tool can you not live without right now?” with a short, no-link explanation of using Claude Code as a project partner and requiring it to verify finished work. Status: LIVE and visibly confirmed. Post permalink: https://www.facebook.com/groups/1576900073381388/posts/1717863569285037/
- **Raspberry Pi Project Share:** Mike currently has one post pending admin approval, so no additional post was submitted. The newest visible item was a DietPi release announcement rather than a request for help.
